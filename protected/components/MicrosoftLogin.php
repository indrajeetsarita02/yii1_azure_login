...
class MicrosoftLogin {
    ...
    function getmsLoginurl($allowAnonymous = '0') {
        $this->modDB = Post::model();
...
//        $url = _URL . $_SERVER['REQUEST_URI'];
        ...
        if (isset($_SESSION['sessionkey'])) {
            ...
            $res = $this->modDB->QuerySingle('SELECT * FROM tblAuthSessions WHERE txtSessionKey = \'' . $_SESSION['sessionkey'] . '\'');
            ...
            if (strtotime($res['dtExpires']) < strtotime('+10 minutes')) {
                ...
                if ($res['txtRefreshToken']) {
                    $oauthRequest = $this->generateRequest('grant_type=refresh_token&refresh_token=' . $res['txtRefreshToken'] . '&client_id=' . _OAUTH_CLIENTID . '&scope=' . _OAUTH_SCOPE);
                    ...
                    if ($reply->error) {
                        if (substr($reply->error_description, 0, 12) == 'AADSTS70008:') {
                            ...
                            $this->modDB->updateData('tblAuthSessions', array('txtRedir' => $url, 'txtRefreshToken' => '', 'dtExpires' => date('Y-m-d H:i:s', strtotime('+5 minutes'))), array('intAuthID' => $res['intAuthID']));
                        ...
                        echo $this->errorMessage($reply->error_description);
                        exit;
                    }
                    $idToken = base64_decode(explode('.', $reply->id_token)[1]);
                    $this->modDB->updateData('tblAuthSessions', array('txtToken' => $reply->access_token, 'txtRefreshToken' => $reply->refresh_token, 'txtIDToken' => $idToken, 'txtRedir' => '', 'dtExpires' => date('Y-m-d H:i:s', strtotime('+' . $reply->expires_in . ' seconds'))), array('intAuthID' => $res['intAuthID']));
            ...
            if (!$allowAnonymous || $_GET['login'] == '1') {
                ...
                $this->modDB->insertData('tblAuthSessions', array('txtSessionKey' => $sessionKey, 'txtRedir' => $url, 'txtCodeVerifier' => $this->oAuthVerifier, 'dtExpires' => date('Y-m-d H:i:s', strtotime('+5 minutes'))));
                // Redirect to Azure AD login page
               return $oAuthURL = 'https://login.microsoftonline.com/' . _OAUTH_TENANTID . '/oauth2/v2.0/' . 'authorize?response_type=code&client_id=' . _OAUTH_CLIENTID . '&redirect_uri=' . urlencode(_URL . '/oauth.php') . '&scope=' . _OAUTH_SCOPE . '&code_challenge=' . $this->oAuthChallenge . '&code_challenge_method=' . $this->oAuthChallengeMethod;
                header('Location: ' . $oAuthURL);
                exit;
            }
        }
        ...
        $this->modDB->runQuery('DELETE FROM tblAuthSessions WHERE dtExpires < \'' . date('Y-m-d H:i:s', $maxRefresh) . '\'');
    }
